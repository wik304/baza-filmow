...
if ($user_id) {
    // Logika dla zalogowanego użytkownika (zapis do bazy danych)
    ...
    // Logika dla gościa (zapis do sesji)
    ...
    if ($key !== false) {
        // Film jest na liście, więc go usuwamy
        unset($_SESSION['guest_lists'][$list_type][$key]);
        echo json_encode(['status' => 'success', 'action' => 'removed']);
    } else {
        // Filmu nie ma na liście, więc go dodajemy
