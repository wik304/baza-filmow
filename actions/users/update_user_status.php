...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['user_id']) || !isset($_POST['is_banned'])) {
...
if ($target_user_role === 'owner' || ($current_user_role === 'admin' && $target_user_role === 'admin')) {
    echo json_encode(['success' => false, 'message' => 'Nie masz uprawnień, aby zbanować tego użytkownika.']);
    exit();
}

$stmt = $conn->prepare("UPDATE users SET is_banned = ? WHERE id = ?");
$stmt->bind_param("ii", $is_banned, $user_id_to_change);
